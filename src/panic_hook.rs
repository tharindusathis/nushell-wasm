// adapted from the console_error_panic_hook, to allow showing the panic
// message to the user, by adding it to the DOM somewhere
use std::panic;
...
fn hook_impl(info: &panic::PanicInfo) {
    ...
    // Finally, expose the panic with `debug.error`!
    error(msg);
}
...
/// Set the panic hook the first time this is called. Subsequent
